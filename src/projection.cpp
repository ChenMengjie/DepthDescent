...
arma::mat banding(arma::vec U, int k){
  ...
  arma::mat UUt(p, p);
  UUt.fill(0);
  arma::uvec nonzero_ind = find(U != 0);
  arma::vec u = U(nonzero_ind);
  arma::mat uut = u*u.t();
  
  for (int i = 0; i < k+1; i++) {
    for(int j = k + i; j < 2*k; j++){
      uut(i, j) = 0; 
    }
  }
  ...
  UUt.submat(nonzero_ind, nonzero_ind) = uut;
...
Rcpp::List move_a_step_with_banding(arma::uvec depth_on_u_move, int Minter_depth_u_move, arma::vec u_move, arma::mat Minter, arma::rowvec project_to_u_move, int k){
    ...
    arma::mat banded = banding(u_move, k);
    arma::mat M = Minter - arma::vectorise(step(0)*banded);
...
Rcpp::List descent_algorithm_matrix_depth_bandable(arma::mat XXT, arma::vec Minitial, arma::mat directions, int ntry, int step_check, int band){
   ...
   for (int i=0; i < ntry; i++) {
      ...
      if(len > 1){
       // Rcpp::Rcout << "Mark 3" << len << std::endl;
        arma::uvec aa = arma::randi<arma::uvec>(1, arma::distr_param(0, len-1));
        u_move = vectorise(u_min.col(aa(0)));  
        arma::uvec all(len);    
        for (int i=0; i < len; i++) {
     ...
     // Rcpp::Rcout << project_to_u_move(0) << std::endl;  
      ...
      if(Minter_depth_u_move >= ncol/2-2) {
          ...
          Rcpp::List M_move_a_step = move_a_step_with_banding(depth_on_u_move, Minter_depth_u_move, u_move, Minter, project_to_u_move, band);
          arma::mat Minitial_new = M_move_a_step["M"];
          Minitial = Minitial_new;
      }
